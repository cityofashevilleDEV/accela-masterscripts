...
| Action# : N/A
|
| Notes   : MHELVICK 12/27/12-updated function getGISInfo to use ApplicationSubmitBefore function
|           MHELVICK 03/12/12-added function loadParcelAttributesTPS to use parcelnum instead of capId
|           MHELVICK 02/10/16-added loadAppSpecific4ACA
|           MHELVICK 02/11/16-added reqDocsEMS() and getDocumentListByEntity()
|           MHELVICK 02/11/16-added line 53: var capIdString = capId.getID1() + "-" + capId.getID2() + "-" + capId.getID3();
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
showDebug = false;
if (appMatch('*/*/*/*') && AInfo['Publically Funded?'] != 'Yes' && AInfo['Owner Occupied?'] != 'Yes' && AInfo['Total Project Valuation'] >= 30000) {
	reqDocs = new Array();
	reqDocs = getDocReqsEMS();
	for (x in reqDocs)
		//start replaced branch: ES_DISPLAY_DOC_MESSAGEGE
	{
		cancel = true;
		showMessage = true;
		comment('Document Type Required: ' + reqDocs[x]);

	}
	//end replaced branch: ES_DISPLAY_DOC_MESSAGEGE;
}

if (appMatch('Permits/Commercial/*/Repair-Replacement') && AInfo['Publically Funded?'] == 'No' && AInfo['Total Project Valuation'] >= 30000) {
	reqDocs = new Array();
	reqDocs = getDocReqsEMS();
	for (x in reqDocs)
		//start replaced branch: ES_DISPLAY_DOC_MESSAGEGE
	...
	if (capContactArray)
		...
		for (yy in capContactArray)
			...
			if (arguments.length == 0 && !cap.isCompleteCap()) { // using capModel to get contacts
				pattrs = capContactArray[yy].getPeople().getAttributes();
				//var pa = capContactArray[yy].getPeople().getAttributes().toArray();
			}
			else {
				pattrs = capContactArray[yy].getCapContactModel().getPeople().getAttributes();
				//var pa = capContactArray[yy].getCapContactModel().getPeople().getAttributes().toArray();
	                }
	                if ( pattrs ) {
	                	var pa = pattrs.toArray();
	                	for (xx1 in pa)
...
function getDocReqsEMS() 
{
    requirementArray = new Array();
	missingDocsArray = new Array();
	
	requirementArray.push("LIEN AGENT DESIGNATION");
	//requirementArray.push("01- Government ID");
	
	var docsList = new Array();

	docsList = getDocumentListByEntity();//Get all Documents on a Record

	var assignDocList = aa.util.newArrayList();

	for (l in requirementArray)
	{	
		logDebug("Looping through required docList.  Iterator = " + l + br);
		var reqDocument = requirementArray[l];
		
		logDebug("Required Document Check: " + reqDocument);		
		docFound = false;
		...
		{	
			...
			logDebug("DEBUG: Doc Category: " +  docCategory);
			
			if (reqDocument.equals(docCategory))
			{
				docFound = true;
				logDebug("Required doc " + docCategory + " has been attached");
			}
		}
		
		logDebug(reqDocument + " docFound= " + docFound);
		
		if (!docFound)
			missingDocsArray.push(reqDocument);
	}
	

	return missingDocsArray;
}

function getDocumentListByEntity() 
{
