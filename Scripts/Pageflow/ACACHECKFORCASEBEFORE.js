...
showMessage = false;
showDebug = false;
// DISABLED: ACACheckForCaseBefore:2
...
if (parcel) {
	...
	//comment('The Parcel = '+ParcelValidatedNumber);
}
...
if (!appMatch('Services/*/*/*') && !appMatch('Permits/Sign/*/*') && paArray['ParcelAttribute.HRC OVERLAY'] == 'Yes') {
  ...
  email('acole@ashevillenc.gov','noreply@ashevillenc.gov','HRC Application: '+aa.publicUser.getPublicUserByPUser("publicUserID")+' Attempted','An on-line HRC application has been attempted by '+aa.publicUser.getPublicUserByPUser("publicUserID")+' and it requires your attention.');
}

if (!appMatch('Services/*/*/*') && !appMatch('Permits/Sign/*/*') && paArray['ParcelAttribute.LANDMARK'] == 'Yes') {
	showMessage = true;
	comment('This application cannot proceed because the parcel is identified as a Landmark. Please come to the permit center to apply for this permit.');
}

if (appMatch('Permits/*/*/*') && paArray['ParcelAttribute.JURISDICTION'] != 'Asheville Corporate Limits') {
	showMessage = true;
	comment('The ADDRESS you have selected does not appear to be in Asheville City Limts');
	cancel = true;
}
